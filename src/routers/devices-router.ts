...
devicesRouter.delete('/devices',
    ...
    async( req: Request, res: Response ) => {
        const userId = req.userId.toString()
        await devicesSessionsService.logoutDevicesSessionsByUser(req.issuedAt,
            userId)
        res.sendStatus(204)
        return
    })
devicesRouter.delete('/devices/:deviceId',
    ...
    async( req: Request, res: Response ) => {
        ...
        const userId = req.userId.toString()
        const logoutDeviceSession = await devicesSessionsService.logoutDeviceSessionByDeviceId(deviceId,
            userId)
