...
usersRouter.get('/',
    ...
    async( req: RequestQuery<QueryInputParams & SearchEmailTerm & SearchLoginTerm>, res: ResponseViewBody<PaginatorType<UserViewModel[]>> ) => {
        const getAllUsers = await usersQueryRepository.returnOfAllUsers(req.query.searchEmailTerm,
...
usersRouter.get('/:id',
    superAdminAuthentication,
    async( req: RequestParamsId<{ id: string }>, res: ResponseViewBody<UserViewModel> ) => {
        const getByIdUser = await usersService.findUserById(req.params.id)
        return getByIdUser === null ? res.sendStatus(404) : res.send(getByIdUser)
    })
